"""
Multi-Stage Training for UCI-HAR Dataset LOSO:
...
from lib.uci_train import train_loso_uci_multi_stage
...
def _upload_results_log_to_wandb(
	...
	try:
		...
		upload_run = wandb.init(
			...
			name=f"uci-loso-five-stage-log-{preprocessing}-{run_suffix}",
		...
		artifact = wandb.Artifact(
			name=f"uci-loso-five-stage-results-{preprocessing}-{run_suffix}",
			type="results-log",
		)
...
def main():
	parser = argparse.ArgumentParser(description='Five-stage LOSO training on UCI-HAR dataset')
	...
	parser.add_argument('--sparsity_weight_bin', type=float, default=0.15,
	...
	parser.add_argument('--subjects', type=str, default=None,
						help='Optional LOSO validation subjects to run, e.g. "1,2,3". '
	...
	results_log_path = project_root / 'log' / f"uci_loso_five_stage_results_{args.preprocessing}.txt"
	results_log_path.parent.mkdir(parents=True, exist_ok=True)
	with open(results_log_path, 'w') as f:
		f.write('UCI-HAR LOSO Five-Stage Training Results\n')
	...
	for val_subject in fold_subjects:
		...
		if args.wandb is False:
			...
			wandb_run = wandb.init(
				...
				name=f"uci-loso-five-stage-val-{val_subject}-{args.preprocessing}-{args.run_name}",
				config={
					'dataset': 'UCI-HAR',
		...
		metrics = train_loso_uci_multi_stage(
			root_path=root_path,
			train_subjects=train_subjects,
			val_subjects=val_subjects,
			wandb_run=wandb_run,
			...
			batch_size=args.batch_size,
			device=device,
			model_path=project_root / 'models' / f"uci_best_model_five_stage_subject{val_subject}_val.pth",
			preprocessing=args.preprocessing,
			sparsity_weight_bin=args.sparsity_weight_bin,
			sparsity_weight_channel=args.sparsity_weight_channel,
	...
	print('=' * 50)
	print('UCI-HAR LOSO Five-Stage Cross-Validation Results')
	print('=' * 50)
	with open(results_log_path, 'a') as f:
		f.write("\n" + '=' * 50 + "\n")
		f.write('Overall UCI-HAR LOSO Five-Stage Results\n')
		f.write('=' * 50 + "\n")
