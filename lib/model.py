...
class GumbelMaskSeparableConvCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # Store hard mask based on bin_logits (not soft probabilities)
        # This ensures mask values are always 0.0 or 1.0
        hard_mask_from_logits = (self.bin_logits.argmax(dim=-1) == 1).float()
        self.last_mask = hard_mask_from_logits.detach()
