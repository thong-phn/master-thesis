"""
Pruning method: Magnitude pruning (L1-Norm)
Three-Stage Channel-Pruning Training for WEAR Dataset LOSO:
    ...
    Stage 2: Calculate L1-Norm of channel weights and calculate stationary mask based on pruning ratio
    Stage 3: Load Stage 1 weights into physically shrunk PrunedSeparableConvCNN and fine tune
"""
...
from lib.wear_train_other_method import train_loso_wear_three_stage_static_pruning
...
def main():
    parser = argparse.ArgumentParser(
        description='Three-stage static L1 channel-pruning LOSO training on WEAR dataset'
    )
    ...
    parser.add_argument('--epochs_stage3', type=int, default=60,
                        help='Number of epochs for stage 3 (Fine tuning with physical shrink model)')
    ...
    results_log_path = project_root / 'log' / f"wear_loso_three_stage_channel_static_results_{args.preprocessing}.txt"
    results_log_path.parent.mkdir(parents=True, exist_ok=True)
    with open(results_log_path, "w") as f:
        f.write("WEAR LOSO Three-Stage Static L1 Channel-Pruning Training Results\n")
    ...
    for val_subject in fold_subjects:
        ...
        wandb_run = wandb.init(
            project="thesis",
            name=f"wear-loso-three-stage-channel-static-val-{val_subject}-{args.preprocessing}",
            config={
                "dataset": "WEAR",
        ...
        fold_model_path = (
            ...
            if args.stage3_model_path is not None
            else project_root / "models" / f"wear_best_model_three_stage_channel_static_subject{val_subject}_val.pth"
        )

        metrics = train_loso_wear_three_stage_static_pruning(
            root_path=root_path,
            train_subjects=train_subjects,
            val_subjects=val_subjects,
            wandb_run=wandb_run,
        ...
        with open(results_log_path, "a") as f:
            ...
            pruning_stats = metrics["stage3"].get("pruning_stats")
    ...
    print("=" * 50)
    print("WEAR LOSO Three-Stage Static Channel-Pruning Cross-Validation Results")
    print("=" * 50)

    with open(results_log_path, "a") as f:
        f.write("\n" + "=" * 50 + "\n")
        f.write("Overall WEAR LOSO Three-Stage Static Channel-Pruning Results\n")
        f.write("=" * 50 + "\n")
